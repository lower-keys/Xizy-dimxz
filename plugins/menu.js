 ...
 *_Limit Tersisa* : %limit
 *_Role* : %role
 *_Level* : %exp/%maxexp
 %totalexp XP secara Total

 *_Tanggal* : %week %weton, %date
 *_Tanggal Islam* : %dateIslamic
 *_Waktu* : %time

 *_Uptime* : %uptime (%muptime)
 *_Database* : %rtotalreg dari %totalreg
...
let handler = async (m, { conn, usedPrefix: _p, args, command }) => {
  ...
  try {
    ...
    if (teks == '404') {
      let judul = `Hi Ada Yang Bisa Gwehj Bantu😅
`.trim()
      const sections = [
      {
    ...
    conn.sendButtonDoc(m.chat, text, `Xizy_MD`,'Credits', '.tqto', m, { quoted: ftroli,
    contextInfo: { 
